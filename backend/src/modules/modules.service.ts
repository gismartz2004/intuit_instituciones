...
import { modulos, niveles, contenidos, InsertModulo, InsertNivel, InsertContenido } from '../shared/schema';
...
export class ModulesService {
    ...
    async getModuleById(id: number) {
        const [mod] = await this.db.select().from(modulos).where(eq(modulos.id, id));
        return mod;
    }
    ...
    async getLevelsByModule(moduleId: number) {
        return this.db.select().from(niveles).where(eq(niveles.moduloId, moduleId)).orderBy(niveles.orden);
    }

    async createContent(data: InsertContenido) {
        const [newContent] = await this.db.insert(contenidos).values(data).returning();
        return newContent;
    }

    async getContentsByLevel(levelId: number) {
        return this.db.select().from(contenidos).where(eq(contenidos.nivelId, levelId));
    }

    async getAllModules() {
        const _modules = await this.db.select().from(modulos);

        const modulesWithDetails = await Promise.all(_modules.map(async (mod) => {
            ...
            const professorAssignments = await this.db.select({
                user: schema.usuarios
            }).from(schema.asignaciones)
            ...
            return {
                ...
                professor: professorAssignments.length > 0 ? professorAssignments[0].user.nombre : "Sin asignar"
            };
        }));
    ...
    async assignUserToModule(data: { estudianteId?: number, profesorId?: number, moduloId: number }) {
        ...
        if (data.estudianteId) {
            ...
            throw new BadRequestException('Debe proporcionar estudianteId o profesorId');
        }

        const existing = await this.db.select().from(schema.asignaciones).where(
            and(
                eq(schema.asignaciones.moduloId, data.moduloId),
                updateCondition
            )
        ).limit(1);
