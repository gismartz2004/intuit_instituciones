...
const BdViewer = forwardRef(({ levelId: propLevelId, isFullscreen }: { levelId?: number, isFullscreen?: boolean }, ref) => {
    ...
    return (
        <div className={cn(
            "w-full bg-slate-50 flex overflow-hidden transition-all duration-500",
            isFullscreen
                ? "h-screen rounded-0 border-0"
                : "h-[calc(100vh-140px)] rounded-[2.5rem] border border-slate-200 shadow-sm"
        )}>
            {/* MAIN CONTENT (Interactive Article View) - 75% */}
            <div className="flex-[3] bg-white h-full overflow-y-auto custom-scrollbar relative">
                ...
                <div className="absolute top-8 left-8 flex items-center gap-2">
                    <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest">{bdTemplate?.nombreModulo}</span>
                    <span className="text-slate-200">/</span>
                    <span className="text-[10px] font-black text-violet-500 uppercase tracking-widest">Fase {currentStep}</span>
                </div>
            </div>
            ...
            <div className="hidden lg:flex flex-1 flex-col bg-slate-50 border-l border-slate-200 h-full overflow-hidden">
                <div className="p-8 border-b border-slate-200 bg-white">
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <Badge className="bg-violet-600 text-[10px] px-2">{currentStep}/12</Badge>
                    </div>
                </div>
